...
const H2H = ({ winners, losers, ...props }) => {
	...
	useEffect(() => {
		async function getData() {
			setData(await playerAPI.getPlayerH2H(winners[0], losers[0]))
		}
		getData()
	}, [winners, losers])
				...
				<Card columnStart={2} columnEnd={3}>
					<Grid gap="0" templateColumns={"1fr 0fr 1fr"}>
						<span style={{ textAlign: 'center' }}>
							...
							<Text variation="success">{data.stats.totals.stats.matches.percentage} % wins</Text>
						</span>
						...
						<span style={{ textAlign: 'center' }}>
							...
							<Text>{100 - data.stats.totals.stats.matches.percentage} % wins</Text>
						</span>
					</Grid>
				</Card>
				...
				{[{ name: 'sets' }, { name: 'games' }, { name: 'tiebreaks' }].map((x, i) =>
					<React.Fragment key={i}>
						<Card columnStart={1} columnEnd={2} className="statsCard">
							{`${data.stats.totals.stats[x.name].wins}/${data.stats.totals.stats[x.name].total} (${data.stats.totals.stats[x.name].percentage}%)`}
						</Card>
						...
						<Card columnStart={3} columnEnd={-1} className="statsCard">
							{`${data.stats.totals.stats[x.name].losses}/${data.stats.totals.stats[x.name].total} (${100 - data.stats.totals.stats[x.name].percentage}%)`}
						</Card>
					</React.Fragment>
				)}
